...
Properties {
	...
	_AudioTex_ST("ST", Vector) = (1,1,0,0)
	...
	_Object("Object X-Scale Y-Y Z-Smoothing", Vector) = (0,0,0,0)
...
uniform float4 _AudioTex_ST;
uniform float4 _OffsetTex_ST;
uniform float4 _Object;
...
float2 map( in float3 pos, float t )
{
	...
	float2 uv = float2(atan2(pos.z, pos.x), length(pos.zx));
	uv = abs(fmod(abs(uv*_AudioTex_ST.xy+ _AudioTex_ST.zw)+1.,2.0)-1.);
	float f = tex2Dlod(_AudioTex, float4(uv, 1., 1.));
	float s = pos.y -f*_TerainHeight;
	
	pos.xz = lerp(pos.xz, abs(fmod(abs(pos.xz)+_Object.x, 2.*_Object.x)- _Object.x), _Object.w);
   	//cross
	float3 p = pos -float3(0., _Object.y, 0.);
   	float c = sdBox(p,float3(.2,.8,.1)*_Object.x);
   	c = opU(c, sdBox(float3(p.x,p.y+.3*_Object.x,p.z),float3(.55,.2,.1)*_Object.x));
	s = smin(s, c, _Object.z+f);
	s = opS(s, sdSphere(_Camera.xyz-poss, 2.));
     /*
   	//gates
   	p = pos;
   	p.z = fmod(p.z,5.0);
...
fixed4 frag (v2f_img i) : COLOR
{
	...
	float3 rd = normalize((float3(p.xy, -rayZ))*mu);
